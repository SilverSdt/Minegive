...
Le projet sera développer en **python**.
...
- [Minegive](#minegive)
  ...
  - [Auteur](#auteur)
  - [Objectif de version](#objectif-de-version)
  ...
  - [Features](#features)
    ...
    - [Non-terminé](#non-terminé)
      - [Création de sous-classe pour les catégorie NBT](#création-de-sous-classe-pour-les-catégorie-nbt)
    ...
    - [Non-commencé](#non-commencé)
      ...
      - [Containerisation du front-end](#containerisation-du-front-end)
...
Les [**milestones**](https://github.com/SilverSdt/Minegive/milestones) représenterons les différentes versions de Minegive.
...
ils doivent être attribuer à la milestones adéquate et possiblement à un membre de l’équipe.

### Tests unitaires

Création de **tests unitaires** pour chaque fonctions et classes du programme.

Ils seront réaliser avec la bibliothèque python [**unittest**](https://docs.python.org/fr/3/library/unittest.html) et auront un répertoire attribué.

### Documentation

Le projet étant en python, la documentation pourras être générer automatiquement grâce à [**Pydoc**](https://docs.python.org/fr/3/library/pydoc.html) qui utilisera les commentaires dans le programme comme référence, génération qui sera faite à la fin de chaque version.
...
Chaques features se verra créer une **nouvelle [branche](https://github.com/SilverSdt/Minegive/branches)** afin de garantir la stabilité de la branche main pour qu’elle reste une version propre et utilisable.

### Pull Requests

Des [**pull requests**](https://github.com/SilverSdt/Minegive/pulls) seront faite afin de demander une validation externe de la qualité du programme fournis, notamment pour les merge qui sont les plus sensibles.
...
A l'heure actuelle, seulement les block et les items on était fait.
...
Le CLI aurait permit de simplifier la création de commande en demandant les divers paramètres et en
vérifiant la validité des informations saisie.

#### Création des routes de l'API

Les routes de l'API aurait permit d'appeler Minegive simplement avec une requête HTTP.
...
#### Containerisation du front-end

Containeriser le front-end aurait pu se faire grâce à l'image [**Django**](https://hub.docker.com/_/django).

La containairisation du front fait suite logique à la containerisation du back.

#### Déploiement automatique avec actions GitHub

Les nouvelles [**release**](https://github.com/SilverSdt/Minegive/releases) aurait était directement intégrer au serveur, permettant de garder à jour l'API
facilement grâce a un fichier yml.

## Ressenti

L'idée de base nous a beaucoup plus. Mais notre choix de projet c'est avéré plus compliqué que prévu.

En effet les commandes Minecraft nous on demandé beaucoup de recherche et un long temps pour la compréhension, se qui nous à ralentit dans notre démarche et ayant abouti à un projet non fini.
